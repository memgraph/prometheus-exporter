...
To add the Memgraph Grafana dashboard to your Grafana instance, you can download the `dashboards/memgraph_prometheus.yaml` file and apply it to your Grafana instance using `helm upgrade` and pass it as a value file, or `helm install` when setting up the monitoring stack, e.g.
```bash
helm upgrade --install kube-prometheus-stack oci://ghcr.io/prometheus-community/charts/kube-prometheus-stack \
  --namespace monitoring \
  --create-namespace \
  -f dashboards/memgraph_prometheus.yaml
```
This values file also exposes Grafana via a NodePort on `32000` (reachable at `http://<node-ip>:32000`).

### Updating the dashboard

If changes are made to the exporter metrics, run:

```bash
tools/generate-dashboards
```

This regenerates both dashboard sets:

- Prometheus/Loki:
  - `dashboards/memgraph_prometheus.json`
  - `dashboards/memgraph_prometheus.yaml`
- VictoriaMetrics/VictoriaLogs:
  - `dashboards/memgraph_victoriametrics.json`
  - `dashboards/memgraph_victoria.yaml`